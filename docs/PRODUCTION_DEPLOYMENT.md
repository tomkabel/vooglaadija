...
6. Polls `https://<domain>/health` until healthy (also verifies the local `:80` path first)
...
- Plain HTTP `:80` is also served (Caddy), so Cloudflare "Flexible" mode works too.
...
- Secrets live only in `./.env` (mode 600) — never in the repo.
