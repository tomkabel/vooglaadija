...
Vooglaadija is a full-stack application that accepts YouTube video URLs, processes download jobs
asynchronously via a background worker, and serves the results through a web dashboard. It features
resilient job processing with circuit breaker protection, intelligent error classification with
retry policies, real-time status updates via Server-Sent Events, and a comprehensive monitoring
stack.
...
Multi-part monorepo: API Server (backend) + Worker (backend) + Frontend (web) + Infrastructure
(infra).
