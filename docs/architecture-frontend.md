...
**Part:** Frontend (`frontend/`) **Type:** Server-rendered web with Tailwind CSS + HTMX

---

## Executive Summary

The frontend uses server-rendered Jinja2 templates with HTMX for dynamic partial updates and SSE for
real-time job status streaming. The Tailwind CSS build pipeline processes source CSS from
`frontend/css/src/` and deploys the output to `app/static/css/`.
