...
// Operators can tune throughput per deployment via BD_MAX_CONCURRENCY
// (defaults to 2); parseTimeout falls back to the default on missing/invalid values.
...
export function createApp() {
  ...
  app.post('/download', async (req, res) => {
    ...
    try {
      // Defence in depth: `download()` re-validates both inputs and is the
      // real security boundary (it also validates URLs derived later, e.g. HLS
      // variants/segments). Validating here as well is deliberate — it is what
      // maps client-supplied garbage to HTTP 400 instead of the 502 that a
      // downloader-side failure produces.
    ...
    // Cancellation channel for the overall request timeout. Releasing the
    // semaphore slot is not enough: without aborting, the Chromium instance,
    // the CDP session and any streamlink/ffmpeg subprocess keep running while a
    ...
    try {
      ...
      // rejects so the finally block releases the semaphore slot instead of
      // holding it forever. Read per-request so tests can override via env.
      const requestTimeout = parseTimeout(
        process.env.BD_REQUEST_TIMEOUT_MS,
        DEFAULT_REQUEST_TIMEOUT_MS,
      );
      const downloadPromise = download(url, output_dir, {
        tier1Timeout,
        tier2Timeout,
      ...
      // rejection. `Promise.race` below still sees the original promise, so a
      // fast rejection is reported normally.
      ...
      // Any non-timeout exit path (success, downloader failure, client abort)
      // also cancels, so nothing outlives the request.
      cancel.abort();
      release();
    }
  });
