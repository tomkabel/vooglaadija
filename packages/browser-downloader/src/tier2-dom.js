...
/**
 * Attempts to activate playback by clicking the first play control that accepts a click.
 *
 * Only a *successful* click stops the search: if an overlay intercepts the click
 * on `video`, later selectors still get a turn. The fallback is restricted to
 * play-labelled buttons — a bare `button` selector matches the first button in
 * the document, which on a third-party page is typically a consent banner or a
 * share dialog rather than the player.
...
async function tryClickPlay(page) {
  ...
  for (const sel of selectors) {
    try {
      ...
      // Click intercepted / element detached / selector unsupported — try the
      // next candidate instead of giving up on playback entirely.
    }
  }
}
...
export async function detectBlob(page, opts = {}) {
  ...
  await page.evaluate(HOOK_SRC, BLOB_CAP).catch(() => {});
        ...
        payload = await page.evaluate(
          async ({ u, cap, timeoutMs }) => {
            ...
            try {
              ...
              // Encode as base64 in-page and decode in Node. Sending the bytes
              // as a JSON number array multiplies the wire size several-fold and
              // can OOM the page + Node process for large blobs.
              let binary = '';
              const CHUNK = 0x8000;
              for (let i = 0; i < bytes.length; i += CHUNK) {
                binary += String.fromCharCode.apply(null, bytes.subarray(i, i + CHUNK));
              }
              return { b64: btoa(binary), type, size: bytes.length };
            } finally {
              clearTimeout(timer);
            }
          },
